    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end">
        <label className="block flex-1">
          <span className="mb-2 block text-sm font-medium text-slate-700">Tìm tác phẩm</span>
          <span className="relative block">
            <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" aria-hidden="true" />
            <input
              value={value.search}
              onChange={(event) => updateValue('search', event.target.value)}
              placeholder="Tên tác phẩm hoặc mô tả"
        ...
        <div className="grid grid-cols-2 gap-3 lg:w-[300px]">
          <label className="block">
            <span className="mb-2 block text-sm font-medium text-slate-700">Giá từ</span>
            <input
              type="number"
              min="0"
          ...
          <label className="block">
            <span className="mb-2 block text-sm font-medium text-slate-700">Đến</span>
            <input
              type="number"
              min="0"
        ...
        <div className="flex items-center justify-between gap-3 lg:pb-0.5">
          {resultCount !== undefined && <span className="text-sm text-slate-500">{resultCount} tác phẩm</span>}
          {hasFilters && (
            <button
              type="button"
