...
export default function PricingPage() {
  ...
  return (
    <div className="overflow-hidden bg-white text-gray-900 antialiased">
        ...
        <div className="mx-auto max-w-5xl text-center">
          ...
          <div className="mt-8 inline-flex items-center gap-2 rounded-lg bg-white/90 p-1.5 shadow-sm ring-1 ring-gray-200">
            <button
              ...
              className={`rounded-md px-4 py-2 text-sm font-bold transition ${billing === 'monthly' ? 'bg-gray-950 text-white shadow-sm' : 'text-gray-500 hover:text-gray-900'}`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-2 rounded-md px-4 py-2 text-sm font-bold transition ${billing === 'yearly' ? 'bg-gray-950 text-white shadow-sm' : 'text-gray-500 hover:text-gray-900'}`}
            >
              {t('pricing.billing.yearly')} <span className="rounded-full bg-gray-200 px-2 py-0.5 text-[10px] font-black uppercase tracking-wide text-gray-950">{t('pricing.billing.save')}</span>
            </button>
          </div>
        ...
        <div className="mx-auto -mt-6 grid max-w-6xl gap-4 md:-mt-8 md:grid-cols-3 md:gap-5">
          {plans.map((plan) => {
            ...
            return (
              <article
                key={plan.name}
              className={`relative flex flex-col rounded-xl border p-6 shadow-sm transition hover:-translate-y-1 hover:shadow-lg sm:p-7 ${plan.featured ? 'border-gray-950 bg-gray-950 text-white shadow-gray-300/80' : 'border-gray-200 bg-white text-gray-900'}`}
              >
                {plan.featured && <div className="absolute -top-3 left-6 rounded-md bg-white px-3 py-1 text-[10px] font-black uppercase tracking-[.12em] text-gray-950">{t('pricing.mostPopular')}</div>}
                <p className={`text-[11px] font-extrabold uppercase tracking-[.14em] ${plan.featured ? 'text-gray-300' : 'text-gray-950'}`}>{plan.kicker}</p>
                ...
                <Link
                  to="/register"
                  className={`mt-7 flex items-center justify-center gap-2 rounded-lg px-5 py-3 text-sm font-bold transition ${plan.featured ? 'bg-white text-gray-950 hover:bg-gray-100' : 'bg-gray-950 text-white hover:bg-black'}`}
                >
      ...
      <section className="bg-gray-950 px-5 py-20 text-white sm:px-8 sm:py-28">
        <div className="mx-auto max-w-6xl">
          <div className="flex flex-col justify-between gap-5 md:flex-row md:items-end">
            <div>
              <p className="text-[11px] font-extrabold uppercase tracking-[.16em] text-gray-300">{t('pricing.comparison.eyebrow')}</p>
          ...
          <div className="mt-12 overflow-hidden rounded-3xl border border-white/10 bg-gray-900">
            ...
            {comparisonGroups.map((group) => (
              <div key={group.title}>
                <div className="bg-black/20 px-4 py-3 text-[10px] font-black uppercase tracking-[.14em] text-gray-300 sm:px-5">{group.title}</div>
      ...
      <section className="px-5 py-20 sm:px-8 sm:py-28">
        <div className="mx-auto max-w-6xl">
          <div className="max-w-2xl">
            <p className="text-[11px] font-extrabold uppercase tracking-[.16em] text-gray-950">{t('pricing.services.eyebrow')}</p>
          ...
          <div className="mt-10 grid gap-4 md:grid-cols-3">
            {services.map((service) => (
              <div key={service.id} className="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
                ...
                <button type="button" className="mt-5 inline-flex items-center gap-1 text-sm font-bold text-gray-900 hover:text-black">{t('pricing.learnMore')} <ArrowRight size={15} /></button>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="px-5 pb-20 sm:px-8 sm:pb-28">
        <div className="mx-auto max-w-6xl overflow-hidden rounded-xl bg-gray-950 px-7 py-12 text-center sm:px-16 sm:py-16">
          <p className="text-[11px] font-extrabold uppercase tracking-[.16em] text-gray-300">{t('pricing.cta.eyebrow')}</p>
          <h2 className="mx-auto mt-4 max-w-3xl text-4xl font-black tracking-tight text-white sm:text-6xl">{t('pricing.cta.title')}</h2>
          <Link to="/register" className="mt-8 inline-flex items-center gap-2 rounded-lg bg-white px-6 py-3.5 text-sm font-bold text-gray-950 transition hover:bg-gray-100">{t('pricing.cta.button')} <ArrowRight size={17} /></Link>
        </div>
      </section>
    </div>
  );
