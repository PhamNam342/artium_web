...
export default function ArtworkDetailPage() {
  ...
  useEffect(() => {
    ...
    artworkService.getArtwork(id)
      ...
      .catch(() => {
        if (active) setError('Không thể tìm thấy tác phẩm này hoặc tác phẩm hiện không còn khả dụng.');
      })
      .finally(() => {
  ...
  if (error || !artwork) {
    return <div className="mx-auto max-w-3xl px-4 py-20 text-center sm:px-6"><h1 className="text-2xl font-bold text-slate-900">Tác phẩm không khả dụng</h1><p className="mt-3 text-slate-600">{error}</p><Link to="/artworks" className="mt-6 inline-flex rounded-full bg-blue-600 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-blue-700">Quay lại danh sách tác phẩm</Link></div>;
  }

  return (
    <div className="bg-white">
      <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:py-10">
        <Link to="/artworks" className="mb-6 inline-flex items-center gap-2 text-sm font-medium text-slate-600 transition hover:text-slate-950"><ArrowLeft className="h-4 w-4" aria-hidden="true" />Tất cả tác phẩm</Link>
        <div className="grid items-start gap-8 lg:grid-cols-[minmax(0,1.35fr)_minmax(320px,0.8fr)] lg:gap-12">
          <div>
            <div className="relative aspect-[4/3] overflow-hidden rounded-2xl bg-slate-100">
              {currentImage ? <img src={currentImage.secureUrl || currentImage.url} alt={currentImage.altText || currentImage.alt || artwork.title} className="h-full w-full cursor-zoom-in object-contain p-4 sm:p-8" onClick={() => setIsLightboxOpen(true)} /> : <div className="flex h-full flex-col items-center justify-center gap-3 text-slate-400"><ImageOff className="h-10 w-10" /><span className="text-sm">Tác phẩm chưa có hình ảnh</span></div>}
              {currentImage && <button type="button" onClick={() => setIsLightboxOpen(true)} className="absolute right-4 bottom-4 rounded-full bg-white/90 p-2.5 text-slate-700 shadow-sm transition hover:bg-white" aria-label="Xem ảnh toàn màn hình"><Maximize2 className="h-4 w-4" /></button>}
              {images.length > 1 && <><button type="button" onClick={() => selectRelativeImage(-1)} className="absolute left-3 top-1/2 -translate-y-1/2 rounded-full bg-white/90 p-2.5 text-slate-700 shadow-sm transition hover:bg-white" aria-label="Ảnh trước"><ChevronLeft className="h-5 w-5" /></button><button type="button" onClick={() => selectRelativeImage(1)} className="absolute right-3 top-1/2 -translate-y-1/2 rounded-full bg-white/90 p-2.5 text-slate-700 shadow-sm transition hover:bg-white" aria-label="Ảnh sau"><ChevronRight className="h-5 w-5" /></button></>}
            </div>
            {images.length > 1 && <div className="mt-4 flex gap-3 overflow-x-auto pb-1">{images.map((image, index) => <button type="button" key={`${image.publicId || image.url}-${index}`} onClick={() => setSelectedIndex(index)} className={`h-20 w-20 shrink-0 overflow-hidden rounded-lg border-2 transition ${selectedIndex === index ? 'border-blue-600' : 'border-transparent hover:border-slate-300'}`} aria-label={`Xem ảnh ${index + 1}`}><img src={image.secureUrl || image.url} alt="" className="h-full w-full object-cover" /></button>)}</div>}
          </div>
          ...
          <article className="lg:sticky lg:top-24">
            ...
            {weight && <p className="mt-1 text-sm text-slate-600">Khối lượng: {weight}</p>}
            <div className="mt-7 border-y border-slate-200 py-5"><p className="text-2xl font-semibold text-slate-950">{formatArtworkPrice(artwork.price, artwork.currency)}</p><p className="mt-2 text-sm text-slate-500">Liên hệ nghệ sĩ để biết thêm thông tin về tác phẩm và phương thức mua.</p></div>
            {artwork.description && <div className="mt-7"><h2 className="text-sm font-semibold uppercase tracking-wider text-slate-900">Về tác phẩm</h2><p className="mt-3 whitespace-pre-line text-sm leading-7 text-slate-600">{artwork.description}</p></div>}
          </article>
        </div>
      </div>

      {isLightboxOpen && currentImage && <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/95 p-4" role="dialog" aria-modal="true" aria-label={`Ảnh của ${artwork.title}`} onClick={() => setIsLightboxOpen(false)}><button type="button" className="absolute right-4 top-4 rounded-full p-2 text-white transition hover:bg-white/10" aria-label="Đóng" onClick={() => setIsLightboxOpen(false)}><X className="h-6 w-6" /></button>{images.length > 1 && <button type="button" onClick={(event) => { event.stopPropagation(); selectRelativeImage(-1); }} className="absolute left-3 rounded-full p-3 text-white transition hover:bg-white/10 sm:left-6" aria-label="Ảnh trước"><ChevronLeft className="h-7 w-7" /></button>}<img src={currentImage.secureUrl || currentImage.url} alt={currentImage.altText || currentImage.alt || artwork.title} className="max-h-[85vh] max-w-[85vw] object-contain" onClick={(event) => event.stopPropagation()} />{images.length > 1 && <button type="button" onClick={(event) => { event.stopPropagation(); selectRelativeImage(1); }} className="absolute right-3 rounded-full p-3 text-white transition hover:bg-white/10 sm:right-6" aria-label="Ảnh sau"><ChevronRight className="h-7 w-7" /></button>}</div>}
    </div>
  );
}
