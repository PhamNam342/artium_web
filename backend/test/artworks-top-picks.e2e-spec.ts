...
describe('Artworks top picks (e2e)', () => {
  ...
  afterAll(async () => {
    if (artworkIds.length > 0) {
      await artworkLikeRepository.delete({ artworkId: In(artworkIds) });
      await artworkRepository.delete({ id: In(artworkIds) });
    }
    if (userIds.length > 0) {
  ...
  it('orders published artworks by like count, then newest creation date', async () => {
    ...
    const response = await request(app.getHttpServer())
      .get('/artworks')
      .query({ search: testRunId, sort: 'top-picks' })
      .expect(200);

    expect(response.body.data.map((artwork: Artwork) => artwork.title)).toEqual([
      newerTopPick.title,
      olderTopPick.title,
      lowerPick.title,
    ]);
